import { getPayload, type Config, type SanitizedConfig, type Where } from 'payload'

export const POSTS_PER_PAGE = 12

type PayloadConfigInput = Config | SanitizedConfig | Promise<SanitizedConfig>

async function resolveConfig(config: PayloadConfigInput): Promise<SanitizedConfig> {
  return Promise.resolve(config) as Promise<SanitizedConfig>
}

export function createQueryHelpers(payloadConfig: PayloadConfigInput) {
